...
function characterCount(string, c) {
  ...
  while (i <= string.length) {
    if (
      string.charAt(i).toLowerCase() == c ||
      string.charAt(i).toUpperCase() == c
    )
      occur++;
    i++;
  }
...
function largestIncrement(numbers) {
  ...
  while (i < numbers.length - 1) {
    incrementsArray[i] = numbers[i + 1] - numbers[i];
    i++;
  }
  incrementsArray.sort((a, b) => a - b);
  return incrementsArray[incrementsArray.length - 1];
}
...
function elementInArray(numbers, x) {
  if (numbers.indexOf(x) > -1) return true;
  else return false;
}
